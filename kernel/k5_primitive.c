     ...
     )
{
    ...
    if (svc_num <= 0 || svc_num > K5_LAST_SVC)  return K5_NO_SERVER;
    if (esb == NULL  || to == NULL)  return K5_NO_ACCESS;
     ...
     ) 
{
    ...
    if (esb == NULL)  return K5_NO_ACCESS;

    memset(esb, 0, K5_MAX_BUF); //清零ESB帧结构,整页

    esb->primitive = K5_WAIT;         //设置服务原语
    esb->dst_port  = k_current_get(); //获得当前运行自身线程的id
